...
const getStoragePath = () => path.join(app.getPath("userData"), storageFileName);
...
const saveStorage = async (data) =>
  fs.writeFile(getStoragePath(), JSON.stringify(data, null, 2), "utf8");
...
const saveSettings = async (settings) =>
  fs.writeFile(getSettingsPath(), JSON.stringify(settings, null, 2), "utf8");
...
const ensureLocalBackup = async (payload) => {
  const backupDir = path.join(
    app.getPath("documents"),
    "MTN-Muhasebe-Yedekler"
  );
  await fs.mkdir(backupDir, { recursive: true });
  const backupFile = path.join(backupDir, "son-yedek.json");
...
const restoreFromSyncLocations = async () => {
  const settings = await loadSettings();
  const localPath = getStoragePath();
  const localStat = await fileStat(localPath);
  const candidates = [];

  if (settings.enableAutoSync && settings.autoSyncPath) {
    candidates.push(path.join(settings.autoSyncPath, storageFileName));
  }

  if (
    settings.enableCloudBackup &&
    settings.cloudBackupPath &&
    (await isOnline())
  ) {
    candidates.push(path.join(settings.cloudBackupPath, storageFileName));
  }

  const candidateStats = await Promise.all(
    candidates.map(async (candidate) => ({
      path: candidate,
      stat: await fileStat(candidate)
    }))
  );

  const available = candidateStats
  ...
  if (!localStat || latest.stat.mtimeMs > localStat.mtimeMs) {
    await fs.copyFile(latest.path, localPath);
    return latest.path;
  }
  return null;
};
...
const syncStorageCopies = async (data) => {
  ...
  const tasks = [];
  const online = settings.enableCloudBackup ? await isOnline() : false;
  ...
  if (settings.enableCloudBackup && settings.cloudBackupPath && online) {
    const target = path.join(settings.cloudBackupPath, storageFileName);
    tasks.push(fs.writeFile(target, payload, "utf8"));
  }

  if (settings.enableCloudBackup && !online) {
    tasks.push(ensureLocalBackup(payload));
  }
...
app.whenReady().then(async () => {
  await restoreFromSyncLocations();
